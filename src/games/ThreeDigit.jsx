...
import ThreeDigittHistoryTable from "./ThreeDigitResultTab";
...
function DigitBall({ label, color }) {
  ...
  return (
    <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-white text-sm ${colors[color] || "bg-gray-400"}`}>
      {label}
    </div>
  );
}
...
function DigitInput({ value, onChange, inputRef, onKeyDown }) {
  return (
    <input
      ref={inputRef}
      type="text"
      ...
      onKeyDown={onKeyDown}
      className="w-6 h-6 border border-gray-200 rounded-lg text-center text-sm font-semibold text-gray-700 focus:outline-none focus:border-purple-400 bg-white"
      placeholder="–"
    />
  );
}
...
export function ThreeDigitDetail() {
  ...
  const showT = (msg, type = "info") => {
   ...
   const doubleABRefs = useRef([]);
const doubleACRefs = useRef([]);
const doubleBCRefs = useRef([]);

const tripleRefs = useRef([]);
  ...
  const fetchGame = async () => {
    try {
      setLoadingGame(true); // ✅ start loader
      ...
      setLoadingGame(false); // ✅ stop loader
    }
  };
  ...
  // ✅ NOW safe
  const isPageLoading = loadingGame || loadingWallet;
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setLoadingWallet(false); // ✅ correct
    }
  }, []);
...
const addBet = (type, digits, price, qty, label = "") => {
  ...
  if (type === "Single") {
    if (label === "A") setSingleA("");
    if (label === "B") setSingleB("");
    if (label === "C") setSingleC("");
  }
  if (type === "Double") {
    setDoubleAB(["", ""]);
    setDoubleAC(["", ""]);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      )}
      {
        ...
        <>
          <div className="p-4 animate-pulse">
            <div className="h-6 bg-black-900 rounded w-1/2 mb-3" />
            <div className="h-20 bg-gray-900 rounded mb-3" />
          ...
          {slots.length === 0 ? (
            ...
            slots.map((slot, i) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold border transition-all whitespace-nowrap ${activeSlotIdx === i
      ...
      <div className="mx-4 mt-3 mb-2 bg-purple-50 border border-purple-200 rounded-2xl px-4 py-3">
        <div className="flex items-start justify-between">
          <div>
            <div className="font-bold text-gray-800 text-sm">{game.name}</div>
          ...
          "
            ...
            <div className="text-xs text-gray-400 mt-1">{game?.code}</div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-4 mb-3 bg-white rounded-2xl border border-gray-100 p-4">
        ...
        {[
  { labels: ["A", "B"], val: doubleAB, set: setDoubleAB, refs: doubleABRefs },
  { labels: ["A", "C"], val: doubleAC, set: setDoubleAC, refs: doubleACRefs },
  { labels: ["B", "C"], val: doubleBC, set: setDoubleBC, refs: doubleBCRefs },
].map(({ labels, val, set, refs }) => {
  ...
  return (
    <div key={labels.join("")} className="flex items-center justify-between mb-2">
      ...
      <button
        ...
        className="bg-purple-100 text-purple-700 font-bold text-xs px-3 py-2 rounded-lg shrink-0 mx-1"
      >
        ADD
      </button>
    </div>
  ...
  {tripleDigits.map((v, i) => (
    <DigitInput
      key={i}
      value={v}
      inputRef={(el) => (tripleRefs.current[i] = el)}
      onChange={(nv) => {
        ...
        // 👉 Auto move next
        if (nv && tripleRefs.current[i + 1]) {
          tripleRefs.current[i + 1].focus();
        }
      }}
      onKeyDown={(e) => {
        // 👉 Backspace go previous
        if (e.key === "Backspace" && !v && tripleRefs.current[i - 1]) {
          tripleRefs.current[i - 1].focus();
        }
      }}
    />
        ...
        <div className="flex">
          {["result", "myorder"].map((t) => (
            <button
              ...
              className={`flex-1 py-3 text-sm relative transition-colors ${activeTab === t ? "font-bold text-gray-900" : "text-gray-400"
                }`}
            >
      ...
      <div className="mx-4 mb-28 bg-white rounded-b-2xl border border-gray-100 border-t-0 px-3 pb-4">
        {activeTab === "result" && (
          <ThreeDigittHistoryTable gameId={game?.id}/>
        )}
        {activeTab === "myorder" && (
          <div className="mt-2">
            {loadingOrders ? (
  ...
  <div className="max-w-[420px] mx-auto flex items-center justify-between px-4 py-3">
    ...
    <button
      ...
      className="bg-purple-600 hover:bg-purple-700 disabled:opacity-40 text-white font-bold px-6 py-3 rounded-full text-sm"
    >
      Pay Now
    </button>
